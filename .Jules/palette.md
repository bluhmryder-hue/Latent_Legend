...
## 2026-05-15 - [Race Condition in Temporary UI Feedback]
**Learning:** In 'sentient-quest', when implementing temporary visual feedback (e.g., changing button text to 'Copied!'), use a guard clause (checking for a '.copied' class or data attribute) to prevent race conditions where rapid clicks might capture the temporary feedback text as the 'original' content.
**Action:** Always wrap temporary UI state transitions in a guard clause that checks if the state is already active.
