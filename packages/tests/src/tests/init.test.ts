...
describe("shadcn init - next-monorepo", () => {
  ...
  it("should create a monorepo with custom preset url", async () => {
    ...
    const initUrl = `${baseUrl}/init?base=radix&style=nova&baseColor=neutral&theme=neutral&chartColor=neutral&iconLibrary=lucide&font=inter&rtl=false&menuAccent=subtle&menuColor=default&radius=default&template=next`
